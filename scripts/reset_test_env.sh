...
#   - l'état persistant du watcher (delta_token.json/file_cache.json dans le volume
#     watcher_state) — sans ça, le watcher reprendrait exactement où il en était sur
#     SharePoint et ne re-détecterait rien (l'état survit maintenant à un simple
#     --force-recreate, contrairement à avant, voir commit b22594b)
#
# Conserve :
#   - les YAML fournisseurs versionnés (atlantic_scga_*, airisol, agenor)
#   - le schéma de base et l'historique des migrations
#
# Le watcher est arrêté pendant l'opération, son état persistant purgé, puis il est
# recréé : au prochain cycle il re-scanne tout SharePoint depuis zéro (re-détection
...
echo ">>> 1/7 Arrêt du watcher (évite tout re-traitement pendant le reset)..."
docker compose stop watcher

echo ">>> 2/7 Truncate des tables applicatives (alembic_version conservé)..."
...
echo ">>> 5/7 Suppression des YAML générés par l'IA (non versionnés)..."
git clean -f config/suppliers/

echo ">>> 6/7 Purge de l'état persistant du watcher (delta_token/file_cache)..."
docker compose run --rm --entrypoint sh watcher -c \
  'rm -f /app/state/delta_token.json /app/state/file_cache.json' \
  || echo "    (purge de l'état watcher ignorée — sans impact sur les re-tests)"

echo ">>> 7/7 Recréation du watcher (re-scan complet au prochain cycle)..."
