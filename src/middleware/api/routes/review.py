...
def export_preview(pending_id: str, request: ExportPreviewRequest) -> dict:
    ...
    rows = build_new_article_rows(
        ...
        result.file_metadata.siren_fournisseur,
    )
    ...
    return {
        ...
        "file_metadata": {
            ...
            "siren_fournisseur": fm.siren_fournisseur,
...
def generate_yaml_for_pending(pending_id: str) -> dict:
    """Déclenche une génération IA fraîche du YAML pour un fichier en attente.

    Toujours un appel IA réel (Gemini) — aucun raccourci de réutilisation d'un
    YAML déjà connu ici.
    """
    meta = _load_pending(pending_id)
    if meta["status"] != "pending":
